...
#     allowed_conditions: List[Ruleset.OntologyCondition] = []
#     if 'Valid terms' in rule:
...
#                     descendant = True
#                 ontology_condition = Ruleset.OntologyCondition(term['term'], descendant, leaf, root)
...
# def check_with_ruleset(sample: List[Dict], ruleset) -> List[ValidationResult.ValidationResultRecord]:
#     submission_result = []
#     for one in sample:
#         one = one['attributes']
#         id_field = 'Data source ID'
...
#             rule_section = ruleset.get_section_by_name(section_name)
#             # apply both standard and material type specific sections
#             if section_name != 'standard' and section_name != one[material_field][0]['value']:
#                 continue
#             section_rules = ruleset[section_name]
...
#                         msg = "Mandatory field " + field_name + " in " + section_name + " section could not be found"
...
#
#                         one_field_result = validate_one_field(one[field_name], rule, section_name, record_id)
#                         if one_field_result:
